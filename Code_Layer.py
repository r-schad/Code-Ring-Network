...
class CodeLayer:
    ...
    '''
    def __init__(self, num_map_units, num_ring_units, code_factor, code_ring_spread=0.02) -> None:
        '''
        ...
        self.num_unique_units = num_ring_units
        self.code_factor = code_factor
        self.num_units = num_ring_units * code_factor
        self.num_ring_units = num_ring_units
        

        self.weights_to_map_from_code = np.random.rand(num_map_units, self.num_units)
        self.weights_to_ring_from_code = self.get_gauss_ring_weights(scale=code_ring_spread)
        

    def get_gauss_ring_weights(self, scale=0.02):
        '''
        ...
        weight_vals_std = weight_vals / np.max(weight_vals)
        ...
        cov_mat = np.ndarray((self.num_ring_units, self.num_unique_units))
        ...
        cov_mat_strip = np.repeat(cov_mat[:, :, np.newaxis], repeats=self.code_factor, axis=2)
